---
title: "ATAC pseudobulk analysis"
author: "Bence Kover"
date: "2026-01-27"
...
bed_df <- data.frame(peak = vars) %>%
  tidyr::separate(peak, into = c("chrom", "coords"), sep = ":") %>%
...
# Save BED (tab-separated, no header)
write.table(
  bed_df,
  "/Users/k23030440/Library/CloudStorage/OneDrive-King'sCollegeLondon/PhD/Year_two/Aim 1/DE_atac/consensus_chromatin_landscape.bed",
  sep = "\t",
  row.names = FALSE,
  col.names = FALSE,
  quote = FALSE
...
write.table(
  bed_df,
  "/Users/k23030440/epitome_code/epitome/data/additional_downloads/consensus_chromatin_landscape/v_0.02/consensus_chromatin_landscape.bed",
  sep = "\t",
  row.names = FALSE,
  col.names = FALSE,
  quote = FALSE
...
# Define function that compares accessibility between a given cell type vs every other cell types
...
# Extracting lineage marker peaks
...
```{r}
#rename gene to peak
all_results$peak <- all_results$gene
#chr start end but keep peak as well
all_results <- all_results %>%
  tidyr::separate(gene, into = c("chr", "coords"), sep = ":") %>%
  tidyr::separate(coords, into = c("start", "end"), sep = "-")
head(all_results)
```
...
# Annotating object for motif enrichment
...
target_peaks$peak <- gsub(":", "-", target_peaks$peak)

all_target_peaks <- all_results
all_target_peaks$peak <- gsub(":", "-", all_target_peaks$peak)

#keep only target_peaks
atac_object_subset = atac_object[rownames(atac_object) %in% all_target_peaks$peak]

enriched.motifs <- FindMotifs(
  object = atac_object_subset,
  features = target_peaks$peak
)

```

# Performing motif enrichment on relevant significant peaks
...
perform_motif_analysis <- function(all_results, grouping_name, direction, output_dir) {
  ...
  target_peaks$peak = target_peaks$peak[grep("^chr", target_peaks$peak)]
...
# Save the normalized peak matrix
...
# Load the normalized peak matrix

```{r}
library(Matrix)
library(readr)

base_path <- "/Users/k23030440/epitome_code/epitome/data/accessibility/v_0.02/"

# Load sparse matrix
normalized_expression <- readMM(paste0(base_path, "normalized_data.mtx"))

# Load row and column names
features <- read.table(paste0(base_path, "accessibility_features.txt"),
                       stringsAsFactors = FALSE)$V1

columns <- read.table(paste0(base_path, "accessibility_columns.txt"),
                      stringsAsFactors = FALSE)$V1

# Assign names
rownames(normalized_expression) <- features
colnames(normalized_expression) <- columns

# Load metadata
meta_data <- read.csv(paste0(base_path, "atac_meta_data.csv"))
...
# Generate pseudobulk PCA and UMAP plots. First PCA colored by cell type and assay
...
# UMAP colored by cell type and sex
...
#ggsave as png
ggsave("/Users/k23030440/Library/CloudStorage/OneDrive-King'sCollegeLondon/PhD/Year_two/Aim 1/DE/ATAC_UMAP_by_assignment.png", p1, width = 6, height = 6, dpi = 300)
#and svg
ggsave("/Users/k23030440/Library/CloudStorage/OneDrive-King'sCollegeLondon/PhD/Year_two/Aim 1/DE/ATAC_UMAP_by_assignment.svg", p1, width = 6, height = 6)


ggsave("/Users/k23030440/Library/CloudStorage/OneDrive-King'sCollegeLondon/PhD/Year_two/Aim 1/DE/ATAC_UMAP_by_sex.png", p2, width = 6, height = 6, dpi = 300)
#and svg
ggsave("/Users/k23030440/Library/CloudStorage/OneDrive-King'sCollegeLondon/PhD/Year_two/Aim 1/DE/ATAC_UMAP_by_sex.svg", p2, width = 6, height = 6)
...
# Extracting and saving chromvar results
```{r}
...
```
# Extracting gene annotations and coordinates for epitome
...
# Extracting cell typing peaks - analogous to cell typing genes
