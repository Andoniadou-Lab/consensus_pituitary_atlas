...
meta_data %>%
  ...
  dplyr::filter(exptype != "multi_rna") %>%
  dplyr::filter(Age_numeric > 10) %>%
  dplyr::filter(Age_numeric < 200) %>%
...
# Defining dream formula and running dream
...
# Save coefs for cell types, which represent average gene expression. These are used for some analyses, including Figure 5C.
...
# As an example result, examining stem cells vs thyrotrophs
...
# Define function that compares expression between a given cell type vs every other cell types
...
# Define groupings relevant for lineage markers
...
# Define function that takes relevant results from all_comparisons, to identify signed intersection of hits for each lineage decision
...
for (i in 1:8) {
  ...
  if (nrow(grouping_results) > 0) {
    ...
    print(grouping_results %>% 
            dplyr::filter(gene %in% tf_list & direction == "up") %>% 
            arrange(geom_mean_adj_pval),
          n = 50)
    
    print("Tfs down")
    print(grouping_results %>% 
            dplyr::filter(gene %in% tf_list & direction == "down") %>% 
            arrange(geom_mean_adj_pval),
          n = 50)
  }
}
...
```
# Comparison against previous results (pre-print version of atlas)
...
all_combined <- bind_rows(
  all_results_v1 %>% dplyr::select(grouping, direction, gene, version),
  all_results_v2 %>% dplyr::select(grouping, direction, gene, version)
)
...
# Split up and down
plot_up <- plot_data %>% dplyr::filter(direction == "up")
plot_down <- plot_data %>% dplyr::filter(direction == "down")
...
# Combine
all_combined <- bind_rows(
  all_results_v1 %>% dplyr::select(grouping, direction, gene, geom_mean_adj_pval, version),
  all_results_v2 %>% dplyr::select(grouping, direction, gene, geom_mean_adj_pval, version)
)
...
both_data <- all_combined %>%
  ...
  dplyr::filter(n_distinct(version) == 2) %>%
  ungroup()

# Spread v1 and v2 into columns
both_wide <- both_data %>%
  dplyr::select(grouping, direction, gene, version, geom_mean_adj_pval) %>%
  pivot_wider(
    names_from = version,
...
version <- "v_0.02"
base_path <- paste0("/Users/k23030440/epitome_code/epitome/data/expression/", version, "/")

# Load matrix
normalized_expression <- readMM(paste0(base_path, "normalized_data.mtx"))

# Load gene names and dataset names
genes <- read.table(paste0(base_path, "genes.txt"), stringsAsFactors = FALSE)$V1
datasets <- read.table(paste0(base_path, "datasets.txt"), stringsAsFactors = FALSE)$V1
...
# Load metadata
meta_data <- read_csv(paste0(base_path, "meta_data.csv"))
```

# Make PCA of normalized_expression colored by assignment and exp_type
...
# Make PCA of normalized_expression colored by sex
...
# Make PCA of sc datasets colored by cell type
...
# Make UMAP of sc datasets colored by cell type
...
# Make PCA and UMAP of sn datasets colored by cell type

```{r}
normalized_expression_sn <- normalized_expression[, meta_data$exptype == "sn"]
keep_genes_sn <- rowSums(normalized_expression_sn > 1) >= 0.05 * ncol(normalized_expression_sn) & rowSums(normalized_expression_sn) >= 5000
normalized_expression_sn <- normalized_expression_sn[keep_genes_sn, ]


pca_data_sn <- prcomp(t(log1p(normalized_expression_sn)), center = TRUE, scale. = TRUE)
var_explained_sn <- pca_data_sn$sdev^2 / sum(pca_data_sn$sdev^2) * 100
plot_data_sn <- data.frame(
  PC1 = pca_data_sn$x[,1],
  PC2 = pca_data_sn$x[,2],
  Assignment = meta_data$assignments[meta_data$exptype == "sn"]
)

p1 <- ggplot(plot_data_sn, aes(x = PC1, y = PC2, fill = Assignment)) +
  geom_point(size =3, alpha = 1, shape = 21, stroke = 0.5, color = "black") +
  theme_minimal() +
  labs(
    x = sprintf("PC1 (%.1f%%)", var_explained[1]),
...
# Extract first 20 PCs
pcs_for_umap <- pca_data_sn$x[,1:10]
...
# Make PCA of multi_rna datasets colored by cell type

```{r}
normalized_expression_multi <- normalized_expression[, meta_data$exptype == "multi_rna"]
keep_genes_multi <- rowSums(normalized_expression_multi > 1) >= 0.05 * ncol(normalized_expression_multi) & rowSums(normalized_expression_multi) >= 5000
normalized_expression_multi <- normalized_expression_multi[keep_genes_multi, ]

pca_data_multi <- prcomp(t(log1p(normalized_expression_multi)), center = TRUE, scale. = TRUE)
var_explained_multi <- pca_data_multi$sdev^2 / sum(pca_data_multi$sdev^2) * 100
plot_data_multi <- data.frame(
  PC1 = pca_data_multi$x[,1],
  PC2 = pca_data_multi$x[,2],
  Assignment = meta_data$assignments[meta_data$exptype == "multi_rna"]
)

p1 <- ggplot(plot_data_multi, aes(x = PC1, y = PC2, fill = Assignment)) +
  geom_point(size =3, alpha = 1, shape = 21, stroke = 0.5, color = "black") +
  theme_minimal() +
  labs(
    x = sprintf("PC1 (%.1f%%)", var_explained[1]),
...
# Make UMAP of multi_rna datasets colored by cell type
...
# Extract first 20 PCs
pcs_for_umap <- pca_data_multi$x[,1:10]
...
#all_markers_df <- all_markers_df %>% dplyr::filter(avg_exp > 5)
...
markers_df <- markers_df %>% dplyr::filter(!is.na(gene))
...
markers<- markers_df %>%
  dplyr::filter(celltype == "Stem_cells") %>%
  dplyr::select(gene)
...
for (ct in celltypes) {
  print(markers_df %>% dplyr::filter(celltype == ct) %>% head(5))
}
```

# Restrict to those that were not used in the initial cellassign workflow
...
for (ct in celltypes) {
  print(new_markers_not_in_cellassign %>% dplyr::filter(celltype == ct)%>% dplyr::filter(avg_exp > 3) %>% arrange(desc(log2fc)) %>% head(9) )
}


#keep only where gene is in new_markers_not_in_cellassign
plot_markers_df <- all_markers_df %>% dplyr::filter(gene %in% new_markers_not_in_cellassign$gene)
#order by lowest pval first
plot_markers_df <- plot_markers_df %>% arrange(pval)


#remove Nr5a1os
plot_markers_df <- plot_markers_df %>% dplyr::filter(gene != "Nr5a1os")
for (ct in celltypes) {
  print(plot_markers_df  %>% dplyr::filter(celltype == ct) %>% dplyr::select(gene,celltype) %>% head(5))
  
}

```

# Compare cell typing markers to previous version (from pre-print)
...
# Keep only relevant columns (adjust if needed)
all_combined <- bind_rows(
  all_results_v1 %>% dplyr::select(celltype,  gene, version),
  all_results_v2 %>% dplyr::select(celltype,  gene, version)
)
...
# Combine
all_combined <- bind_rows(
  all_results_v1 %>% dplyr::select(celltype, gene, pval, version),
  all_results_v2 %>% dplyr::select(celltype, gene, pval, version)
)
...
# Spread v1 and v2 into columns
both_wide <- both_data %>%
  dplyr::select(celltype, gene, version, pval) %>%
  pivot_wider(
    names_from = version,
    values_from = pval
  )
