...
#how many sra_id in each comp_sex
print("female")
print(length(unique(meta_data[meta_data$Comp_sex==0,]$SRA_ID)))
print("male")
print(length(unique(meta_data[meta_data$Comp_sex ==1,]$SRA_ID)))
...
df_final <- df %>% dplyr::filter(abs(logFC) > 1)
df_final  <- df_final  %>%
  dplyr::select(-occurs)
...
# Make a heatmap of genes that occur in the most cell types, but showing this in a balanced way (18 for male bias, 18 for female bias). Also printing the number of unique genes.
...
df_sex_genes_filtered <- df_sex_genes_filtered %>% dplyr::filter(abs(logFC) > 0)
...
df_sex_genes_filtered <-df_sex_genes_filtered %>% dplyr::filter(occurs > 4)
...
length(unique(df_sex_genes_filtered$gene))
...
df_sex_genes <- df_sex_genes %>% dplyr::filter(gene %in% df_sex_genes_filtered$gene)
...
# Only keep those with avg abs logFC > 0 and compute per-gene avg
final_results <- final_results %>%
  group_by(gene) %>%
  mutate(avg_abs_log2fc = mean(abs(logFC), na.rm = TRUE)) %>%
  ungroup()

# Remove genes starting with Gm, ENS, or ending in Rik
final_results <- final_results %>%
  dplyr::filter(!grepl("^Gm|^ENS|Rik$", gene))

# Determine majority sex per gene (which sex has higher mean abs logFC)
majority_sex_df <- final_results %>%
  group_by(gene, sex) %>%
  summarise(mean_abs_log2fc = mean(abs(logFC), na.rm = TRUE), .groups = "drop") %>%
  group_by(gene) %>%
  slice_max(mean_abs_log2fc, n = 1, with_ties = FALSE) %>%
  ungroup() %>%
  dplyr::select(gene, majority_sex = sex)

# Join back into final_results (left_join is safer than right_join here)
final_results <- final_results %>%
  left_join(majority_sex_df, by = "gene")
...
final_results<- final_results %>% dplyr::filter(gene %in% genes_of_interest1)
...
all_results <- all_results %>% dplyr::filter(abs(logFC) > 1)
all_results_v1 <- all_results_v1 %>% dplyr::filter(abs(logFC) > 1)
...
# Split up and down
plot_up <- plot_data %>% dplyr::filter(sex == "male")
plot_down <- plot_data %>% dplyr::filter(sex == "female")
