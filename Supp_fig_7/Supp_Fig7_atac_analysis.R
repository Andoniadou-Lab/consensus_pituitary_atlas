...
meta2 = readxl::read_excel("/Users/k23030440/Library/CloudStorage/OneDrive-King'sCollegeLondon/PhD/Year_two/Aim 1/DE_atac/pituitary_atlas.xlsx")
...
motifs_sub_matrix <- motifs_sub_matrix[rownames(motifs_sub_matrix) %in% all_results$gene, ]

#for each column extract the peaks where its != 0 and then run

annotations_res <- c()
motifs_res <- c()
n_peaks_for_motif_df = data.frame(Motif=character(), N_Peaks=integer(), stringsAsFactors=FALSE)
for (i in 1:ncol(motifs_sub_matrix)) {
  # Get the column name
  motif_name <- colnames(motifs_sub_matrix)[i]
  
  # Get the indices of non-zero values
  non_zero_indices <- which(motifs_sub_matrix[, i] != 0)
  
  # If there are no non-zero values, skip to the next iteration
  if (length(non_zero_indices) == 0) {
    next
  }
  
  # Extract the corresponding peaks
  peaks <- rownames(motifs_sub_matrix)[non_zero_indices]
  ...
  motifs_res <- c(motifs_res, motif_genes[i])
  n_peaks_for_motif_df <- rbind(n_peaks_for_motif_df, data.frame(Motif=motif_genes[i], N_Peaks=length(peaks), stringsAsFactors=FALSE))
}

# Filter out any NULL or empty data frames from annotations_res and motifs_res
#valid_indices <- sapply(annotations_res, function(x) is.data.frame(x) && nrow(x) > 0)
#valid_annotations_res <- annotations_res[valid_indices]
#valid_motifs_res <- motifs_res[valid_indices]

if (length(valid_annotations_res) == 0) {
  stop("No valid annotation summaries were generated. Cannot create heatmap.")
}

# Combine all valid annotation summaries into a single data frame
all_annotation_summaries <- do.call(rbind, annotations_res)

# Create a vector of motif names, repeating each motif name by the number of rows
num_rows_for_rep <- sapply(annotations_res, nrow)
all_annotation_summaries$Motif <- rep(valid_motifs_res, num_rows_for_rep)
...
motif_sums = rowSums(annotation_matrix)
row_normalized_matrix <- sweep(annotation_matrix, 1, colSums(annotation_matrix), FUN = "/")
...
Heatmap(row_normalized_matrix,
        ...
        heatmap_legend_param = list(title = "Fraction of peaks")
)
