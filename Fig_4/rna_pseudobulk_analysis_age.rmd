...
final_results_to_save = final_results %>% dplyr::filter((adj.P.Val < 0.05))
#print total sig
...
#for our information, how many genes are significant in at least 5 cell types
print("Number of genes significant and up in at least 5 cell types:")
print(nrow(final_results_pan_pit[(final_results_pan_pit$gene_count >= 5) & final_results_pan_pit$avg_logFC > 0, ]))
print("Number of genes significant and down in at least 5 cell types:")
print(nrow(final_results_pan_pit[(final_results_pan_pit$gene_count >= 5) & final_results_pan_pit$avg_logFC < 0, ]))
...
#subset to stem cells
final_summary_df <- final_summary_df[final_summary_df$cell_type == "Stem_cells", ]
table(final_summary_df$change_pattern)
...
for (pattern in unique(final_summary_df$change_pattern)) {
  ...
  example_row <- final_summary_df[final_summary_df$change_pattern == pattern, ][3, ]
  gene_name <- example_row$gene
  cell_type <- example_row$cell_type
  message(paste("Example for pattern", pattern, "-> Gene:", gene_name, ", Cell Type:", cell_type))
  data_example <- get_values(cell_type, gene_name, meta_data, normalized_expression)
...
#decrease stagnate
save_gene_plots_dual("Stem_cells", "H19", 
...
#stagnate decrease
save_gene_plots_dual("Stem_cells", "Panx1", 
...
#steadily decrase Ezh2
save_gene_plots_dual("Stem_cells", "Ezh2", 
...
#Rab3c - stagnate - incrase
save_gene_plots_dual("Stem_cells", "Rab3c", 
...
```{r}
# Add temporal patterns to aging_genes list
aging_res = read.csv("/Users/k23030440/epitome_code/epitome/data/aging/v_0.02/aging_genes.csv")
temporal_res = read.csv("/Users/k23030440/Library/CloudStorage/OneDrive-King'sCollegeLondon/PhD/Year_two/Aim 1/temporal_patterns/temporal_patterns.csv", 
                        header = TRUE, stringsAsFactors = FALSE)

# Merge using dplyr (handles the column name mismatch: 'genes' vs 'gene')
library(dplyr)

aging_res <- aging_res %>%
  left_join(
    temporal_res %>% select(gene, cell_type, change_pattern),
    by = c("genes" = "gene", "cell_type" = "cell_type")
  ) %>%
  mutate(change_pattern = ifelse(is.na(change_pattern), "minimal_logFC", change_pattern))

# Check the result
head(aging_res)
table(aging_res$change_pattern)

#save it back as aging_genes_temporal.csv
write.csv(aging_res, "/Users/k23030440/epitome_code/epitome/data/aging/v_0.02/aging_genes_temporal.csv", row.names = FALSE)
```
```{r}
#table for Stem_cells
table(aging_res$change_pattern[aging_res$cell_type == "Stem_cells"])
```
